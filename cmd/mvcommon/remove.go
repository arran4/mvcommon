...
type Remove struct {
	*Skill
	...
	CommandAction func(c *Remove) error
}

type UsageDataRemove struct {
	*Remove
	Recursive bool
}

func (c *Remove) Usage() {
	err := executeUsage(os.Stderr, "remove_usage.txt", UsageDataRemove{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Remove) UsageRecursive() {
	err := executeUsage(os.Stderr, "remove_usage.txt", UsageDataRemove{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}
...
func (c *Skill) NewRemove() *Remove {
	...
	set.BoolVar(&v.force, "force", false, "Force removal")
	set.Usage = v.Usage

	v.CommandAction = func(c *Remove) error {

		err := SkillsRemove(c.name, c.force)
		if err != nil {
