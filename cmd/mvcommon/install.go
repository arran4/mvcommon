...
func (c *Skill) NewInstall() *Install {
	...
	set.StringVar(&v.scope, "scope", "user", "The scope to install to user or project")

	set.StringVar(&v.agent, "agent", "", "The agent to install for e.g. copilot cursor")

	set.BoolVar(&v.force, "force", false, "Force installation replacing locally modified skills")
	set.Usage = v.Usage

	v.CommandAction = func(c *Install) error {

		err := SkillsInstall(c.source, c.scope, c.agent, c.force, c.optionalNames...)
		if err != nil {
